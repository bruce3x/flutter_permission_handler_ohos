...
export class PermissionManager {
  ...
  private successCallback: RequestPermissionsSuccessCallback;

  private requestResults:Map<number, number>;
  ...
  private determinePermissionStatus(permission: number, successCallback: CheckPermissionSuccessCallback): void {
    if (permission == PermissionConstants.PERMISSION_GROUP_NOTIFICATION) {
      this.checkPermission(successCallback, 'ohos.permission.NOTIFICATION_CONTROLLER');
      return;
    }
    ...
    PermissionUtils.getManifestNames(permission, (names) => {
      ...
      for (let name of names) {
        ...
        let status: abilityAccessCtrl.GrantStatus  = this.atManager.verifyAccessTokenSync(this.tokenId, name);
  ...
  requestPermissions(permissions: ArrayList<number>, ability: UIAbility,
    ...
    this.getPermissionToRequest(permissions, (permissionsToRequest) => {
          ...
          this.atManager.requestPermissionsFromUser(ability.context, permissionsToRequest.convertToArray(), (error, result) => {
            ...
            for (let i = 0; i < permissionNames.length; i++) {
              ...
              if (permission == PermissionConstants.PERMISSION_GROUP_UNKNOWN)
                continue;
              let isSuccess: number  = authResult[i];
              if (permission == PermissionConstants.PERMISSION_GROUP_MICROPHONE) {
                this.requestResults.set(PermissionConstants.PERMISSION_GROUP_MICROPHONE, PermissionUtils.toPermissionStatus(isSuccess));
                this.requestResults.set(PermissionConstants.PERMISSION_GROUP_SPEECH, PermissionUtils.toPermissionStatus(isSuccess));
              } else if (permission == PermissionConstants.PERMISSION_GROUP_LOCATION_ALWAYS) {
                this.requestResults.set(PermissionConstants.PERMISSION_GROUP_LOCATION_ALWAYS, PermissionUtils.toPermissionStatus(isSuccess));
              } else if (permission == PermissionConstants.PERMISSION_GROUP_LOCATION) {
                this.requestResults.set(PermissionConstants.PERMISSION_GROUP_LOCATION_WHEN_IN_USE, PermissionUtils.toPermissionStatus(isSuccess));
                this.requestResults.set(PermissionConstants.PERMISSION_GROUP_LOCATION, PermissionUtils.toPermissionStatus(isSuccess));
              } else {
                this.requestResults.set(permission, PermissionUtils.toPermissionStatus(result));
              }
  ...
  private getPermissionToRequest(permissions: ArrayList<number>, callback: Callback<ArrayList<any>>): void {
    let permissionsToRequest: ArrayList<any>  = new ArrayList();
  ...
  private checkBluetoothPermissionStatus(successCallback: CheckPermissionSuccessCallback): void {
    PermissionUtils.getManifestNames(PermissionConstants.PERMISSION_GROUP_BLUETOOTH, (permissions) => {
